...
function fillPerson(data){
    ...
    var isDirtyCounter = data.isDirtyCounter;

    $("#isDirtyCounter").text(isDirtyCounter);
...
function updatePerson() {
    ...
    $.getJSON(url, null, succeeded);
};

function checkIsDirtyPerson(){
    var dirtyCounter = $("#isDirtyCounter").text()
    var personSelect = $("#personId");
    var idFromSelect = personSelect.val();

    var url = "/isPersonDirty";
    var succeeded = function(data){
        if (dirtyCounter != data){
        debugger;
            intializePersonSelect(idFromSelect);
        }
        setTimeout(checkIsDirtyPerson, 1000);
    };
    ...
    $.getJSON(url, parameters, succeeded);
}
